...
class Gen():
    ...
    def __init__(self):
        self.usageCount = 0
        self.cookie = ""
        self.sensorGen = Sensor()
        self.creation = False

    def __create_cookie(self):
        self.creation = True
        logger.log("Preparing - Grabbing _abck cookie.")
        payload = {
            'sensor_data': self.sensorGen.generateSensorData()
        }
        s = requests.Session()
        s.post('https://www.adidas.co.uk/_bm/_data', json=payload)
        while "~0" not in s.cookies['_abck']: 
            sleep(1)
            s.post('https://www.adidas.co.uk/_bm/_data', json=payload)
        self.cookie = s.cookies['_abck']
        self.usageCount = 0
        self.creation = False

    def get_cookie(self):
        if not self.cookie and self.creation == False:
            self.__create_cookie()
        else:
            while not self.cookie:
                sleep(0.5)
        self.usageCount += 1
        if self.usageCount > 6:
            self.__create_cookie()
            self.usageCount += 1
            return self.cookie
        else:
